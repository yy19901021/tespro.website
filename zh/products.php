<?php 
	$title = "泰易电子：产品";
?>
<?php 
	$BaseUrl = "";
	include("../includes/universal.php");
	include("header.php");
?>
<div id="ProductsTitle" class="PageTitle"><div class="PageTitleBrg"><img src="images/Product_Title_zh.png" alt="一切皆为标记" /></div></div><div class="Shadow1"></div><div class="Shadow2"></div><div class="Shadow3"></div>
<div id="Content"><!--Start of content-->
	<div class="Products">
		<!---
		<div class="HalfWidth LeftAligned">

		
			<h2>通讯光电头</h2>
			<ul>
				<li><a href="zh/TP-USB-IEC.php">TP-USB-IEC 电表光电头（USB接口）</a></li>
				<li><a href="zh/TP-USB-ANSI.php">TP-USB-ANSI 电表光电头（USB接口）</a></li>
				<li><a href="zh/TP-232-IEC.php">TP-232-IEC 电能表通用读写光电头（RS232接口）</a></li>
				<li><a href="zh/TP-232-ANSI.php">TP-232-ANSI 电能表通用读写光电头（RS232接口）</a></li>									<li><a href="zh/TP-USB-IR645.php">TP-USB-IR645 红外通讯光电头</a></li>
				<li><a href="zh/TP-HHU.php">TP-HHU Optical Probe</a></li>

			</ul>
			<h2>校验光电头</h2>
			<ul>
				<li><a href="zh/TP-11-14.php">TP-11、TP-14 电表校验光电头</a></li>
				<li><a href="zh/TP-15.php">TP-15 双频校验光电头</a></li>
				<li><a href="zh/TP-16.php">TP-16 进口现校仪光电头</a></li>
				<li><a href="zh/TP-17A.php">TP-17A 机械、电子表校验光电头</a></li>
				<li><a href="zh/TP-18.php">TP-18 窄型校验光电头</a></li>
				<li><a href="zh/TP-18K.php">TP-18K 替代线夹的窄型校验光电头</a></li>
				<li><a href="zh/TP-11-EMC.php">TP-11-EMC 抗扰型EMC实验光电头</a></li>
				<li><a href="zh/TP1_option_table.php">校验光电头插头选用表</a></li>
			</ul>
			<h2>光电头附件</h2>
			<ul>
				<li><a href="zh/TP-GS08.php">TP-GS08 校验光电头活动支架</a></li>
				<li>TF-L5/A3 Plug Adapter</li>
				<li><a href="zh/TF-USB485L.php">TF-USB485L 三口USB转换测试线</a></li>			
			</ul>
		</div>
		<div class="HalfWidth LeftAligned">
			<h2 title="电表测试台系列">电表测试仪器</h2>
			<ul>				
				<li><a href="zh/TA-261.php">TA-261 电表通讯口测试器</a></li>
				<li><a href="zh/TA-320.php">TA-320 断电特性测试仪</a></li>
				<li><a href="zh/TA-420.php">TA-420 通用电表仿真测试平台</a></li>
				<li><a href="zh/ET-2200.php">ET-2200 规约兼容性及一致性测试软件</a></li>
			</ul>
			<h2 title="电表测试台系列">电表测试台</h2>
			<ul>
				<li><a href="zh/TG-MTS-CS.php">TG-MTS-CS 标准端口电能表测试台</a></li>
			</ul>
			<h2 title="电表测试台系列">电表电源设备</h2>
			<ul>
				<li><a href="zh/TD-300.php">TD-300 系列三相电表走字电源</a></li>
				<li><a href="zh/TD-6200.php">TD-6200 三相逆变程控电源</a></li>
			</ul>
			
		</div>

		<div class="Clear"></div>



		<hr size="5" color="red" />
		--->


		<div class="HalfWidth LeftAligned">
			<h2>便携式仪器系列</h2>
			<ul>
				<li><a href="zh/TA-800.php">TA-800 多规约计量数据分析仪</a></li>
				<li><a href="zh/TA-262.php">TA-262 计量通信故障综合测试仪</a></li>
				<li><a href="zh/TA-261.php">TA-261 电表通讯口测试器 </a></li>
			</ul>
			
			<h2>电能计量测试设备</h2>
			<ul>
				<li><a href="zh/TG-MTS-CN.php">TG-MTS-CN 电表测试台</a></li>
				<li><a href="zh/TG-MS-3P.php">三相表维修测试台</a></li>
				<li><a href="zh/TA-430-460.php">TA-430/460 系列中试综合测试台</a></li>
				<li><a href="zh/TC-PCB-01.php">PCB 自动检测台</a></li>
			</ul>
			<h2>逆变电源设备</h2>
			<ul>
				<li><a href="zh/TD-1300.php">TD-1300 单三相逆变电源</a></li>
				<li><a href="zh/TD-2300.php">TD-2300 双三相逆变电源</a></li>
				<li><a href="zh/TD-6300.php">TD-6300 三相逆变校表功率源 </a></li>
				<li><a href="zh/TD-50-100A.php">TD-50-100A 三相逆变校表功率源 </a></li>
			</ul>
		</div>
		<div class="HalfWidth LeftAligned">
			<h2>电表通讯光电头</h2>
			<p>泰易自动化设备有限公司从2002年开始建立光电头全系列产品，以领先的技术积累，坚持创新和不

断优化，为用户提供全面、工艺成熟、耐用可靠的光电头系列产品。首创了USB光电头及RS232接

口取电光电头等专利产品，年年创新推出新品。</p>
			<ul>
				<li><a href="zh/TP-IEC.php">IEC读写光电头</a></li>
				<li><a href="zh/TP-ANSI.php">ANSI读写光电头</a></li>
				<li><a href="zh/TP-USB-IR645.php">DL/T645 标准调制光电头</a></li>
				<li><a href="zh/TP-Handheld.php">手持设备光电头</a></li>
				<li><a href="zh/TP-BT.php">蓝牙/无线传输光电头系列</a></li>
			</ul>
			<h2>校验光电头</h2>
			<ul>
				<li><a href="zh/TP-11.php">TP-11 电表校验光电头</a></li>
				<li><a href="zh/TP-13.php">TP-13 电表校验光电头</a></li>
				<li><a href="zh/TP-15.php">TP-15 双频校验光电头</a></li>
				<li><a href="zh/TP-16.php">TP-16 进口现校仪光电头</a></li>
				<li><a href="zh/TP-17.php">TP-17 进口现校仪光电头</a></li>
				<li><a href="zh/TP-18.php">TP-18 窄型校验光电头</a></li>
				<li><a href="zh/TP-PS.php">TP-PS 通用光电头活动支架</a></li>
				<li><a href="zh/TP-11-EMC.php">TP-11-EMC 抗扰型EMC实验光电头</a></li>
				<li><a href="zh/TP1_option_table.php">光电头插头选用表</a></li>
			</ul>
		</div>
		<div class="Clear"></div>
	</div>
</div><!--End of content-->

<?php include("footer.php") ?>