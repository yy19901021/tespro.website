<?php
	$title = "Tespro Eletronics - Optical Probes and Meter Test Equipment";
?>
<?php 
	$BaseUrl = "";
	include("../includes/universal.php");
	include("header.php");
?>
<div id="HomepageTitle" class="PageTitle"><div class="PageTitleBrg"><img src="images/home_TP-UsbOpticalProbe_en.jpg" 
	alt="TP-USB Optical Probe For Meter Communication(IEC standard)" /></div><div class="Shadow1"></div>
	<div class="Shadow2"></div>
	<div class="Shadow3"></div>
</div>
<div id="Content"><!--Start of content-->
	<div class="Products">
		<div class="HalfWidth LeftAligned">
			<h1>What We Do</h1>
			<p>Founded in 2002, Tespro has been the expert in providing testing instruments and equipment and all series of optical probes for both utilities and meter manufacturers.  Based on the 8 years’ experience and fruits Tespro will continue to make its efforts on the instrumentations and AMR/AMI appilcations. </p>
			<p>&nbsp;</p><p>&nbsp;</p>
		</div>
        <div id="OurCustomers" class="HalfWidth LeftAligned">
			<h1>Our Customers</h1>
				<a href="http://www.sgcc.com.cn/"><img src="images/logos/state-grid.jpg"
                alt="State Grid" /></a><a href="http://www.csg.cn" style="margin: 0px 100px;"><img src="images/logos/china-south-grid.jpg" 
                alt="China Southern Power Grid" /></a><a href="http://www.landisgyr.com.cn/"><img src="images/logos/landis-gyr.jpg"
                alt="Landis &amp; Gyr" /></a><a href="http://www.elster.com/"><img src="images/logos/elster.jpg" 
                alt="Elster" /></a><a href="http://www.itron.com" style="margin: 0 60px;"><img src="images/logos/itron.jpg" alt="Itron" /></a><a href="http://www.edmi-meters.com"><img src="images/logos/edmi.jpg" 
                alt="EDMI" /></a>
        </div>
		<div class="Clear"></div>
	</div>
	<div id="OurCustomers">

	</div>
</div><!--End of content-->
<?php include("footer.php") ?> 